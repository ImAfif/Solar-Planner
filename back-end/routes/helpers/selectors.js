...
const getPanelByInputRange = (inputRange, moduleType, db) => {

  const queryString = `SELECT * FROM solar_panels WHERE max_power_Wp > $1 AND max_power_Wp <= $2 AND technology = $3;`
  ...
  return db
    .query(queryString, [low_range, high_range, moduleType])
    .then(res => res.rows)
